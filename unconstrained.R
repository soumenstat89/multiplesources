...
folderName = paste0("unconstrained_M", i, "_", ndraws,"run_", ts, sep = "")
...
for(jj in 1:M) rr[jj]=dim(XX[[jj]])[2]
  ...
  sigmaOfProposal.taue = 0.15
  sigmaOfProposal.taus = 0.4
  ...
  taus = 1
  beta = rep(1, r)
  gamma=c(beta,rnorm(k1,mean=0,sd=0.1))
  
  logtaue = log(taue)
  logtaus = log(taus)
  log.target.curr=ftauetaus.log.present(exp(logtaue), exp(logtaus), XT, mat1, V.curl)
  ...
  cat(c('taue', 'taus', paste('beta.', 1:r, sep='')), 
      paste('theta1.', 1:r, sep=''), paste('theta2.', 1:(k1-r), sep=''),
      sep=',', file=fname1, append=TRUE)
  ...
  while(continueMCMC) 
  {
    ...
    Sig.gamma.inv[upper.tri(Sig.gamma.inv, diag = F)]=t(Sig.gamma.inv)[upper.tri(t(Sig.gamma.inv), diag=F)]
    
    gamma0hat=taue*Sig.gamma.inv%*%rbind(t(XT), mat1)%*%nucthat
    ...
    # draw taue and taus
      ...
      log.target.cand=ftauetaus.log.present(exp(logtaue.cand), exp(logtaus.cand), XT, mat1, V.curl)
  ...
  dimnames(post.hb) = list(c('taue', 'taus', paste('beta_', 1:r, sep=''),
                             paste('theta1_', 1:r, sep=''),  paste('theta2_', 1:(k1-r), sep='')), 
  ...
  dimnames(cor.comp) = list(c('taue', 'taus', paste('beta.', 1:rr[i], sep=''), 
                              paste('theta1.', 1:rr[i], sep=''),  
                              paste('theta2.', 1:(k1-rr[i]), sep='')),
                            c('taue', 'taus', paste('beta.', 1:rr[i], sep=''), 
                              paste('theta1.', 1:rr[i], sep=''),  
                              paste('theta2.', 1:(k1-rr[i]), sep='')))

   cor.mat=cor.comp[c(3:(2*rr[i]+2)),c(3:(2*rr[i]+2))]
    
  cor.mat.full= cor.comp
...
write.csv(cor.mat, file=paste0(folderName, '/Correlation.matrices.csv', sep=''), quote = F, row.names=T)
